...
The mouse wheel event is sent to the visual element under the mouse when the
mouse scroll wheel value changes. A WheelEvent uses the default mouse event
propagation path: it trickles down, bubbles up and can be cancelled. Disabled
elements won't receive this event by default.

### Static Properties

[scrollDeltaPerTick](UIElements.WheelEvent-scrollDeltaPerTick.html)|  The
magnitude of WheelEvent.delta that corresponds to exactly one tick of the
scroll wheel.  
---|---  
  
### Properties

[delta](UIElements.WheelEvent-delta.html)|  The amount of scrolling applied
with the mouse wheel.  
---|---  
  
### Constructors

[WheelEvent](UIElements.WheelEvent-ctor.html)|  Constructor. Use GetPooled()
to get an event from a pool of reusable events.  
---|---  
  
### Protected Methods

[Init](UIElements.WheelEvent.Init.html)|  Resets the event members to their
initial values.  
---|---  
  
### Static Methods

[GetPooled](UIElements.WheelEvent.GetPooled.html)|  Gets an event from the
event pool and initializes it with the given values. Use this function instead
of creating new events. Events obtained using this method need to be released
