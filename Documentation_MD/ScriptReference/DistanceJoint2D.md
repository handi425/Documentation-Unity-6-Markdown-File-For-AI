...
Note that unlike the SpringJoint2D component, the distance separating the
objects is truly fixed and does not allow for any stretching.  
  
Additional resources: [HingeJoint2D](HingeJoint2D.html) class,
[SliderJoint2D](SliderJoint2D.html) class, [SpringJoint2D](SpringJoint2D.html)
class.

### Properties

[autoConfigureDistance](DistanceJoint2D-autoConfigureDistance.html)| Should
the distance be calculated automatically?  
---|---  
[distance](DistanceJoint2D-distance.html)| The distance separating the two
ends of the joint.  
[maxDistanceOnly](DistanceJoint2D-maxDistanceOnly.html)| Whether to maintain a
maximum distance only or not. If not then the absolute distance will be
maintained instead.  
