...
#
[DisposeSentinel](Unity.Collections.LowLevel.Unsafe.DisposeSentinel.html).Create(AtomicSafetyHandle,DisposeSentinel,int)
...
When `DisposeSentinel` is created, the call stack is stored in it to log a
descriptive error when a memory leak is detected.
