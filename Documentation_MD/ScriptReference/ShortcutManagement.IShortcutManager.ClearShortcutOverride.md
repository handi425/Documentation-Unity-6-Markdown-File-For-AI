...
#
[IShortcutManager](ShortcutManagement.IShortcutManager.html).ClearShortcutOverride
...
`InvalidOperationException` if the active profile is read-only, i.e. when
...
activeProfileId.html). Throws `InvalidArgumentException` when `shortcutId` is
not an available shortcut ID, i.e. when
