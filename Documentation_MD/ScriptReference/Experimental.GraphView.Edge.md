...
[candidatePosition](Experimental.GraphView.Edge-candidatePosition.html)| The
edge's end position while it's being created.  
---|---  
[defaultColor](Experimental.GraphView.Edge-defaultColor.html)| Default edge
color.  
[edgeControl](Experimental.GraphView.Edge-edgeControl.html)| The VisualElement
child of Edge that draws the lines and does the hit detection.  
[edgeWidth](Experimental.GraphView.Edge-edgeWidth.html)| Edge width.  
[ghostColor](Experimental.GraphView.Edge-ghostColor.html)| The color of the
ghost edge, which is the edge that appears snapped to a nearby port while an
edge is being created.  
[input](Experimental.GraphView.Edge-input.html)| Connected input port.  
[isGhostEdge](Experimental.GraphView.Edge-isGhostEdge.html)| Is this edge a
ghost edge, which is the edge that appears snapped to a nearby port while an
edge is being created.  
[output](Experimental.GraphView.Edge-output.html)| Connected output port.  
[PointsAndTangents](Experimental.GraphView.Edge.PointsAndTangents.html)| The
edge's points and tangents.  
[selectedColor](Experimental.GraphView.Edge-selectedColor.html)| Color of edge
while selected.  
[showInMiniMap](Experimental.GraphView.Edge-showInMiniMap.html)| Whether the
GraphElement is shown in the minimap. For Edge, this property is always set to
false.  
  
### Constructors

[Edge](Experimental.GraphView.Edge-ctor.html)| Edge's constructor.  
---|---  
  
### Public Methods

[ContainsPoint](Experimental.GraphView.Edge.ContainsPoint.html)| Checks if
point is on top of edge.  
---|---  
[OnPortChanged](Experimental.GraphView.Edge.OnPortChanged.html)| Called when a
port on the edge is changed.  
[Overlaps](Experimental.GraphView.Edge.Overlaps.html)| Check if edge overlaps
rectangle.  
[UpdateEdgeControl](Experimental.GraphView.Edge.UpdateEdgeControl.html)|
Update the edge's EdgeControl.  
  
### Protected Methods

[CreateEdgeControl](Experimental.GraphView.Edge.CreateEdgeControl.html)|
Create the EdgeControl.  
---|---  
[DrawEdge](Experimental.GraphView.Edge.DrawEdge.html)| Draw the edge's lines.  
[OnCustomStyleResolved](Experimental.GraphView.Edge.OnCustomStyleResolved.html)|
