...
#  [LightingSettings](LightingSettings.html).filterTypeIndirect
...
Specifies whether the Editor calculates the final global illumination light
bounce at the same resolution as the baked lightmap.

When this is set to `None`, the Progressive Lightmapper does not filter
indirect lighting. When this is set to any other value, the Progressive
Lightmapper applies the specified filter kernel to indirect lighting.  
...
the **Filtering > Indirect** property in the Lighting Settings Asset
Inspector.  
