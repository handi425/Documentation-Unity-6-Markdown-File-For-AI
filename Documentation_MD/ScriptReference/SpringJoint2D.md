...
Joint that attempts to keep two [Rigidbody2D](Rigidbody2D.html) objects a set
distance apart by applying a force between them.

Note that unlike DistanceJoint2D, the length of the joint can stretch and
oscillate.  
  
Additional resources: [DistanceJoint2D](DistanceJoint2D.html),
[HingeJoint2D](HingeJoint2D.html), [SliderJoint2D](SliderJoint2D.html).

### Properties

[autoConfigureDistance](SpringJoint2D-autoConfigureDistance.html)| Should the
distance be calculated automatically?  
---|---  
[dampingRatio](SpringJoint2D-dampingRatio.html)| The amount by which the
spring force is reduced in proportion to the movement speed.  
[distance](SpringJoint2D-distance.html)| The distance the spring will try to
keep between the two objects.  
[frequency](SpringJoint2D-frequency.html)| The frequency at which the spring
oscillates around the distance distance between the objects.  
