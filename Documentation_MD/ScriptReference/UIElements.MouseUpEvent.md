...
The mouse up event is sent to a visual element when a mouse button is released
inside the element. A MouseUpEvent uses the default mouse event propagation
path: it trickles down, bubbles up and can be cancelled. Disabled elements
won't receive this event by default.

### Constructors

[MouseUpEvent](UIElements.MouseUpEvent-ctor.html)|  Constructor. Avoid
...
[Init](UIElements.MouseUpEvent.Init.html)|  Resets the event members to their
initial values.  
---|---  
  
### Static Methods

[GetPooled](UIElements.MouseUpEvent.GetPooled.html)|  Gets an event from the
