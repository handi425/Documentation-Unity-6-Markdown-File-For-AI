...
By default it will mask all Sorting Layers. A custom range of Sorting Layers
can be set. If a SortingGroup is present, it will act local to the
SortingGroup.

### Properties

[alphaCutoff](SpriteMask-alphaCutoff.html)| The minimum alpha value used by
the mask to select the area of influence defined over the mask's sprite.  
---|---  
[backSortingLayerID](SpriteMask-backSortingLayerID.html)| Unique ID of the
sorting layer defining the end of the custom range.  
[backSortingOrder](SpriteMask-backSortingOrder.html)| Order within the back
sorting layer defining the end of the custom range.  
[frontSortingLayerID](SpriteMask-frontSortingLayerID.html)| Unique ID of the
sorting layer defining the start of the custom range.  
[frontSortingOrder](SpriteMask-frontSortingOrder.html)| Order within the front
sorting layer defining the start of the custom range.  
[isCustomRangeActive](SpriteMask-isCustomRangeActive.html)| Mask sprites from
front to back sorting values only.  
[maskSource](SpriteMask-maskSource.html)| The source used for generating the
mask for this SpriteMask.  
[sprite](SpriteMask-sprite.html)| The Sprite used to define the mask.  
[spriteSortPoint](SpriteMask-spriteSortPoint.html)| Determines the position of
the Sprite used for sorting the SpriteMask.  
