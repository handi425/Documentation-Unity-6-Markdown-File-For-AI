...
Implements interfaces:[IKeyboardEvent](UIElements.IKeyboardEvent.html)
...
The typical keyboard event loop is as follows: \- When a key is pressed, a
[KeyDownEvent](UIElements.KeyDownEvent.html) is sent. \- If the key is held
down for a duration determined by the OS, another KeyDownEvent with the same
data is sent. While the key is held down, the event is sent repeatedly at a
frequency determined by the OS. \- When the key is released, a
[KeyUpEvent](UIElements.KeyUpEvent.html) is sent.  
  
By default, keyboard events trickle down and bubble up. disabled elements
won't receive these events.  
