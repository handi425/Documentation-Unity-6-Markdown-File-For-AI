...
A [CompositeCollider2D](CompositeCollider2D.html) merges other Colliders
together when their
[Collider2D.compositeOperation](Collider2D-compositeOperation.html) is
anything other than
[Collider2D.CompositeOperation.None](Collider2D.CompositeOperation.None.html),
that is, whenever a composite operation is selected.  
  
When a Composite Collider uses a Collider, the Editor will ignore and not show
the [Collider2D.sharedMaterial](Collider2D-sharedMaterial.html),
[Collider2D.isTrigger](Collider2D-isTrigger.html) &
[Collider2D.compositeOperation](Collider2D-compositeOperation.html)
properties. The same properties on the
[CompositeCollider2D](CompositeCollider2D.html) will be used instead. You
should set these properties on the Composite Collider to merge all Colliders
into the Composite Collider.  
  
**NOTE** : This [Collider2D](Collider2D.html) cannot be disabled/enabled with
the [Behaviour.enabled](Behaviour-enabled.html) property. Any changes to that
property will be ignored.

### Properties

[edgeRadius](CompositeCollider2D-edgeRadius.html)| Controls the radius of all
edges created by the Collider.  
---|---  
[generationType](CompositeCollider2D-generationType.html)| Specifies when to
generate the Composite Collider geometry.  
[geometryType](CompositeCollider2D-geometryType.html)| Specifies the type of
geometry the Composite Collider should generate.  
[offsetDistance](CompositeCollider2D-offsetDistance.html)| Vertices are offset
by this distance when compositing multiple physic shapes. Any vertices between
shapes within this distance are combined.  
[pathCount](CompositeCollider2D-pathCount.html)| The number of paths in the
Collider.  
[pointCount](CompositeCollider2D-pointCount.html)| Gets the total number of
points in all the paths within the Collider.  
[useDelaunayMesh](CompositeCollider2D-useDelaunayMesh.html)| When the value is
true, the Collider uses an additional Delaunay triangulation step to produce
the Collider mesh. When the value is false, this additional step does not
occur.  
[vertexDistance](CompositeCollider2D-vertexDistance.html)| Controls the
minimum distance allowed between generated vertices.  
  
### Public Methods

[GenerateGeometry](CompositeCollider2D.GenerateGeometry.html)| Regenerates the
Composite Collider geometry.  
---|---  
[GetCompositedColliders](CompositeCollider2D.GetCompositedColliders.html)|
When any Collider2D is using any composite operation other than
Collider2D.CompositeOperation.None then it is used by this
CompositeCollider2D.  
[GetPath](CompositeCollider2D.GetPath.html)| Gets a path from the Collider by
its index.  
[GetPathPointCount](CompositeCollider2D.GetPathPointCount.html)| Gets the
number of points in the specified path from the Collider by its index.  
