...
#  [PBXProject](iOS.Xcode.PBXProject.html).SetBuildProperty
...
public void SetBuildProperty(IEnumerable<string> targetGuids, string name,
string value);
...
---|---  
targetGuids | The GUIDs of the targets, such as the one returned by [GetUnityFrameworkTargetGuid](iOS.Xcode.PBXProject.GetUnityFrameworkTargetGuid.html).  
...
Sets a build property to the given value in all build configurations for the
specified target(s).
...
quoted if they contain spaces.
    ...
    public class Sample_SetBuildProperty  
    {
        ...
        {  
            ...
            // Get the GUID that you want to set a build property on
            string unityFrameworkTargetGuid = pbxProject.GetUnityFrameworkTargetGuid();  
      
            // Set "ENABLE_BITCODE" to "NO" for UnityFramework target
