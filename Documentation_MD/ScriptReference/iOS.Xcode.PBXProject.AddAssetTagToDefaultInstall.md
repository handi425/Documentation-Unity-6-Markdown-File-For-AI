...
#  [PBXProject](iOS.Xcode.PBXProject.html).AddAssetTagToDefaultInstall
...
**Note** : The function does nothing if there are no files that use the given
asset tag across the project.
    ...
    public class Sample_AddAssetTagToDefaultInstall  
    {
        [PostProcessBuild]
        ...
        {  
            ...
            // Add the [Asset](VersionControl.Asset.html) Tag to list of tags to be downloaded during initial installation
            // Note that the function does not add the asset tag if there are no files in the project using it
            pbxProject.AddAssetTagToDefaultInstall(mainTargetGuid, "exampleTag");  
