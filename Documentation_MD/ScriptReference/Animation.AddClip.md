...
#  [Animation](Animation.html).AddClip
...
public void AddClip([AnimationClip](AnimationClip.html) clip, string newName);
...
public void AddClip([AnimationClip](AnimationClip.html) clip, string newName,
int firstFrame, int lastFrame, bool addLoopFrame = false);
...
Adds `clip` to the only play between `firstFrame` and `lastFrame`. The new
clip will also be added to the animation with name `newName`.
