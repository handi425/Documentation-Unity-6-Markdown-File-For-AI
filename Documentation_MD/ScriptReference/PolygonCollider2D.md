...
Collider for 2D physics representing an arbitrary polygon defined by its
vertices.
...
[autoTiling](PolygonCollider2D-autoTiling.html)| Determines whether the
PolygonCollider2D's shape is automatically updated based on a SpriteRenderer's
tiling properties.  
---|---  
[pathCount](PolygonCollider2D-pathCount.html)| The number of paths in the
polygon.  
[points](PolygonCollider2D-points.html)| Corner points that define the
collider's shape in local space.  
[useDelaunayMesh](PolygonCollider2D-useDelaunayMesh.html)| When the value is
...
[CreatePrimitive](PolygonCollider2D.CreatePrimitive.html)| Creates as regular
primitive polygon with the specified number of sides.  
---|---  
[GetPath](PolygonCollider2D.GetPath.html)| Gets a path from the Collider by
its index.  
[GetTotalPointCount](PolygonCollider2D.GetTotalPointCount.html)| Return the
total number of points in the polygon in all paths.  
[SetPath](PolygonCollider2D.SetPath.html)| Define a path by its constituent
points.  
