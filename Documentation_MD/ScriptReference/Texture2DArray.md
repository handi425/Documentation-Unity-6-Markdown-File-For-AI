...
Modern graphics APIs (e.g. D3D10 and later, OpenGL ES 3.0 and later, Metal
etc.) support "texture arrays", which is an array of same size & format
textures. From the shader side, they are treated as a single resource, and
sampling them needs an extra coordinate that indicates which array element to
sample from.  
  
Typically texture arrays are useful as an alternative for texture atlases, or
in other cases where objects use a set of same-sized textures (e.g. terrains).  
  
Currently in Unity texture arrays do not have an import pipeline for them, and
must be created from code, either at runtime or in editor scripts. Using
[Graphics.CopyTexture](Graphics.CopyTexture.html) is useful for fast copying
of pixel data from regular 2D textures into elements of a texture array. From
editor scripts, a common way of creating serialized texture array is to create
it, fill with data (either via
[Graphics.CopyTexture](Graphics.CopyTexture.html) from regular 2D textures, or
via [SetPixels](Texture2DArray.SetPixels.html) or
[SetPixels32](Texture2DArray.SetPixels32.html)) and save it as an asset via
[AssetDatabase.CreateAsset](AssetDatabase.CreateAsset.html).  
  
Note that not all platforms and GPUs support texture arrays; for example
Direct3D9 does not. Use [SystemInfo.supports2DArrayTextures](SystemInfo-
supports2DArrayTextures.html) to check. Also, this class does not support
Texture2DArray creation with a Crunch compression
[TextureFormat](TextureFormat.html).

### Static Properties

[allSlices](Texture2DArray-allSlices.html)| Read Only. This property is used
as a parameter in some overloads of the CommandBuffer.Blit, Graphics.Blit,
CommandBuffer.SetRenderTarget, and Graphics.SetRenderTarget methods to
indicate that all texture array slices are bound. The value of this property
is -1.  
---|---  
  
### Properties

[activeMipmapLimit](Texture2DArray-activeMipmapLimit.html)| The number of high
resolution mipmap levels from the texture that Unity doesn't upload to the
GPU. (Read Only)  
---|---  
[depth](Texture2DArray-depth.html)| Number of elements in a texture array
(Read Only).  
[format](Texture2DArray-format.html)| Texture format (Read Only).  
[ignoreMipmapLimit](Texture2DArray-ignoreMipmapLimit.html)| This property
causes a texture to ignore all texture mipmap limit settings.  
[mipmapLimitGroup](Texture2DArray-mipmapLimitGroup.html)| The name of the
texture mipmap limit group that this texture is associated with. (Read Only)  
  
### Constructors

[Texture2DArray](Texture2DArray-ctor.html)| Create a new texture array.  
---|---  
  
### Public Methods

[Apply](Texture2DArray.Apply.html)| Copies changes you've made in a CPU
texture to the GPU.  
---|---  
[CopyPixels](Texture2DArray.CopyPixels.html)| Copies pixel data from another
texture on the CPU.  
[GetPixelData](Texture2DArray.GetPixelData.html)| Gets the raw data from a
texture.  
[GetPixels](Texture2DArray.GetPixels.html)| Gets the pixel color data for a
mipmap level of a slice as Color structs.  
[GetPixels32](Texture2DArray.GetPixels32.html)| Gets the pixel color data for
a mipmap level of a slice as Color32 structs.  
[SetPixelData](Texture2DArray.SetPixelData.html)| Sets the raw data of an
entire mipmap level directly in CPU memory.  
[SetPixels](Texture2DArray.SetPixels.html)| Sets the pixel colors of an entire
mipmap level of a slice.  
[SetPixels32](Texture2DArray.SetPixels32.html)| Sets the pixel colors of an
entire mipmap level of a slice.  
