...
In example, `shader=lit` will be a query filter block with
[QueryBlock.name](Search.QueryBlock-name.html) equal to `shader`,
[QueryBlock.value](Search.QueryBlock-value.html) equal to `lit` and the
[QueryBlock.op](Search.QueryBlock-op.html) equal to `=`.
...
[Apply](Search.QueryBlock.Apply.html)| This method is invoked when a selected
search proposition must be applied to the builder search query.  
---|---  
