...
Class for handling cube maps, Use this to create or modify existing [cube map
assets](../Manual/class-Cubemap.html).
...
[desiredMipmapLevel](Cubemap-desiredMipmapLevel.html)| The mipmap level that
the streaming system would load before memory budgets are applied.  
---|---  
[format](Cubemap-format.html)| The format of the pixel data in the texture
(Read Only).  
[loadedMipmapLevel](Cubemap-loadedMipmapLevel.html)| The mipmap level that is
currently loaded by the streaming system.  
[loadingMipmapLevel](Cubemap-loadingMipmapLevel.html)| The mipmap level that
the mipmap streaming system is in the process of loading.  
[requestedMipmapLevel](Cubemap-requestedMipmapLevel.html)| The mipmap level to
load.  
[streamingMipmaps](Cubemap-streamingMipmaps.html)| Determines whether mipmap
streaming is enabled for this Texture.  
[streamingMipmapsPriority](Cubemap-streamingMipmapsPriority.html)| Sets the
relative priority for this Texture when reducing memory size to fit within the
memory budget.  
  
### Constructors

[Cubemap](Cubemap-ctor.html)| Create a new empty cubemap texture.  
---|---  
  
### Public Methods

[Apply](Cubemap.Apply.html)| Copies changes you've made in a CPU texture to
the GPU.  
---|---  
[ClearRequestedMipmapLevel](Cubemap.ClearRequestedMipmapLevel.html)| Resets
the requestedMipmapLevel field.  
[CopyPixels](Cubemap.CopyPixels.html)| Copies pixel data from another texture
on the CPU.  
[GetPixel](Cubemap.GetPixel.html)| Gets the pixel color at coordinates (x, y).  
[GetPixelData](Cubemap.GetPixelData.html)| Gets the raw data from a texture.  
[GetPixels](Cubemap.GetPixels.html)| Gets the pixel color data for a mipmap
level of a face as Color structs.  
[IsRequestedMipmapLevelLoaded](Cubemap.IsRequestedMipmapLevelLoaded.html)|
Checks to see whether the mipmap level set by requestedMipmapLevel has
finished loading.  
[SetPixel](Cubemap.SetPixel.html)| Sets the pixel color at coordinates (x,y).  
[SetPixelData](Cubemap.SetPixelData.html)| Sets the raw data of an entire
mipmap level of a face directly in CPU memory.  
[SetPixels](Cubemap.SetPixels.html)| Sets the pixel colors of an entire mipmap
level of a face.  
[SmoothEdges](Cubemap.SmoothEdges.html)| Performs smoothing of near edge
regions.  
[UpdateExternalTexture](Cubemap.UpdateExternalTexture.html)| Updates Unity
cubemap to use different native cubemap texture object.  
  
### Static Methods

[CreateExternalTexture](Cubemap.CreateExternalTexture.html)| Creates a Unity
cubemap out of externally created native cubemap object.  
---|---  
