...
#  [PBXProject](iOS.Xcode.PBXProject.html).GetPBXProjectPath
...
**string** The path to the PBX project file. This path can be opened later
using the [ReadFromFile](iOS.Xcode.PBXProject.ReadFromFile.html) function.
...
**Note:** On VisionOS and macOS platforms this will return `BUILDPATH/Unity-
iPhone.xcodeproj/project.pbxproj`, which isn't correct. Its not recommended to
use this method when developing for visionOS or macOS.
    ...
    public class Sample_GetPBXProjectPath  
    {
        [PostProcessBuild]
        ...
        {  
            ...
            // Initialize [PBXProject](iOS.Xcode.PBXProject.html) by using GetPBXProjectPath to create [PBXProject](iOS.Xcode.PBXProject.html) object
            ...
            // Get the GUID for the UnityFramework using it's name 
            string unityFrameworkGuid = pbxProject.TargetGuidByName("UnityFramework");  
      
            // The unityFrameworkGuid can be used later to specify the UnityFramework as the target when manipulating the pbxproj file
