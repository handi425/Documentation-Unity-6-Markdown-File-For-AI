...
Unity does the following in all `multi_compile` keyword sets:
  ...
  * Removes `keywordNames` if the data field under `SelectOrRemove` does not match the value of `condition`.
    ...
    [SelectOrRemove(true, keywordNames: "FeatureA")]
    bool hasFeatureA;
...
[SelectOrRemoveAttribute](ShaderKeywordFilter.SelectOrRemoveAttribute-
ctor.html)| Either include or remove the specified shader keywords in the
