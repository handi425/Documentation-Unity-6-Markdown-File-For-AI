...
#  [UnityWebRequest](Networking.UnityWebRequest.html).uri
...
Same as [url](Networking.UnityWebRequest-url.html), but has less validation
and pre-processing, so is much faster to set. Reading it's value is, however,
more expensive because the new Uri instance is created, so if you want to get
the URL your request ended up to, it's recommended to use
[url](Networking.UnityWebRequest-url.html), unless an Uri object is what you
need.
