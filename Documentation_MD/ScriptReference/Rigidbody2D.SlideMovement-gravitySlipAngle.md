...
#
[Rigidbody2D.SlideMovement](Rigidbody2D.SlideMovement.html).gravitySlipAngle
...
When the gravity movement causes a contact with a
[Collider2D](Collider2D.html), slippage maybe occur if the surface angle is
...
**NOTE:** Slippage will only occur if no initial `velocity` is passed to the
[Rigidbody2D.Slide](Rigidbody2D.Slide.html) method.  
  
Additional resources:
[Rigidbody2D.SlideMovement.surfaceSlideAngle](Rigidbody2D.SlideMovement-
surfaceSlideAngle.html), [Rigidbody2D.Slide](Rigidbody2D.Slide.html) and
