...
Each entry represents a status of a finger touching the screen.
    ...
    public class Example : [MonoBehaviour](MonoBehaviour.html)
    {
        ...
        {
            var fingerCount = 0;
            foreach ([Touch](Touch.html) touch in [Input.touches](Input-touches.html))
            {
                if (touch.phase != [TouchPhase.Ended](TouchPhase.Ended.html) && touch.phase != [TouchPhase.Canceled](TouchPhase.Canceled.html))
                {
                    fingerCount++;
                }
            }
