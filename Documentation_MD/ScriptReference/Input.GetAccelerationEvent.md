...
#  [Input](Input.html).GetAccelerationEvent
...
public static [AccelerationEvent](AccelerationEvent.html)
GetAccelerationEvent(int index);

### Description

Returns specific acceleration measurement which occurred during last frame.
(Does not allocate temporary variables).
...
more](../Manual/Input.html)).
    ...
    public class Example : [MonoBehaviour](MonoBehaviour.html)
    {
        ...
        {
            [Vector3](Vector3.html) acceleration = [Vector3.zero](Vector3-zero.html);
            for (var i = 0; i < [Input.accelerationEventCount](Input-accelerationEventCount.html); ++i)
            {
                [AccelerationEvent](AccelerationEvent.html) accEvent = [Input.GetAccelerationEvent](Input.GetAccelerationEvent.html)(i);
