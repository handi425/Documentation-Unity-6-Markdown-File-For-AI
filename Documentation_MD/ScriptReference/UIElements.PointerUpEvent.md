...
The last pressed button may or may not be the same button that triggered the
[PointerDownEvent](UIElements.PointerDownEvent.html).  
  
A PointerUpEvent uses the default pointer event propagation path: it is
trickled down, bubbled up and can be cancelled. Disabled elements won't
receive this event by default.  
  
See [PointerEventBase<T0>](UIElements.PointerEventBase_1.html) to see how
PointerUpEvent relates to other pointer events.

### Constructors

[PointerUpEvent](UIElements.PointerUpEvent-ctor.html)|  Constructor. Avoid
...
[Init](UIElements.PointerUpEvent.Init.html)|  Resets the event members to
