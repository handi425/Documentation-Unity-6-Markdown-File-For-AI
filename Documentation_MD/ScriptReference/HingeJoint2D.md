...
Joint that allows a Rigidbody2D object to rotate around a point in space or a
point on another object.

Additional resources: [DistanceJoint2D](DistanceJoint2D.html),
[SliderJoint2D](SliderJoint2D.html), [SpringJoint2D](SpringJoint2D.html),
[JointAngleLimits2D](JointAngleLimits2D.html).

### Properties

[jointAngle](HingeJoint2D-jointAngle.html)| The current joint angle (in
degrees) with respect to the reference angle.  
---|---  
[jointSpeed](HingeJoint2D-jointSpeed.html)| The current joint speed.  
[limits](HingeJoint2D-limits.html)| Limit of angular rotation (in degrees) on
the joint.  
[limitState](HingeJoint2D-limitState.html)| Gets the state of the joint limit.  
[motor](HingeJoint2D-motor.html)| Parameters for the motor force applied to
the joint.  
[referenceAngle](HingeJoint2D-referenceAngle.html)| The angle (in degrees)
referenced between the two bodies used as the constraint for the joint.  
[useConnectedAnchor](HingeJoint2D-useConnectedAnchor.html)| Controls whether
the connected anchor is used or not.  
[useLimits](HingeJoint2D-useLimits.html)| Should limits be placed on the range
of rotation?  
[useMotor](HingeJoint2D-useMotor.html)| Should the joint be rotated
automatically by a motor torque?  
  
### Public Methods

[GetMotorTorque](HingeJoint2D.GetMotorTorque.html)| Gets the motor torque of
