...
#  [PBXProject](iOS.Xcode.PBXProject.html).ReadFromString
...
Current contents of the project are discarded.
    ...
    public class Sample_ReadFromString  
    {
        [PostProcessBuild]
        ...
        {  
            ...
            string pbxProjectContent = File.ReadAllText(projectPath);  
      
            // Create a [PBXProject](iOS.Xcode.PBXProject.html) object and read from the string
            [PBXProject](iOS.Xcode.PBXProject.html) pbxProject = new [PBXProject](iOS.Xcode.PBXProject.html)();
            pbxProject.ReadFromString(pbxProjectContent);  
      
            // Get the target GUID you want to make modifications on
            string mainTargetGuid = pbxProject.GetUnityMainTargetGuid();  
      
            // Modify the project, such as adding a build propety
            pbxProject.AddBuildProperty(mainTargetGuid, "exampleProperty", "exampleValue");  
            
            // Convert the modified project back to a string
            string updatedPbxProjectContent = pbxProject.WriteToString();  
      
            // Save the updated project string back to the project.pbxproj file
