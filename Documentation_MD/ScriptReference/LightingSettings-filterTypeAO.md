...
#  [LightingSettings](LightingSettings.html).filterTypeAO
...
Specifies the filter kernel that the Progressive Lightmapper uses for ambient
occlusion. (Editor only).

When this is set to `None`, the Progressive Lightmapper does not filter
ambient occlusion. When this is set to any other value, the Progressive
Lightmapper filters the computed ambient occlusion with the specified filter.  
