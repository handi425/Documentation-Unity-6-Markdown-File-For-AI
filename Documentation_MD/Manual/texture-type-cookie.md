...
Language : English
  ...
  * [English](/Manual/texture-type-cookie.html)
  * [中文](/cn/current/Manual/texture-type-cookie.html)
  * [日本語](/ja/current/Manual/texture-type-cookie.html)
  * [한국어](/kr/current/Manual/texture-type-cookie.html)
...
The **Cookie** texture type formats the texture asset so it’s suitable to use
as a light cookie in the Built-in **Render Pipeline** A series of operations
...
See in [Glossary](Glossary.html#Renderpipeline). For this texture type, Unity
updates the [Texture Shape](class-TextureImporter.html#textureshape)
automatically based on the **Light Type** you select:
...
Note that you must create your source texture with an appropriate layout:
cookies for Point Lights must be laid out as cubemaps, and cookies for
Directional Lights and Spot Lights must be laid out as regular 2D Textures.  
  
If you apply a cookie to a Directional Light, you can use a cookie configured
for a Directional Light, or a cookie configured for a Spot Light. A
Directional Light using a cookie configured for a Directional Light repeats
the cookie in a tiled pattern all over the scene. A Directional Light using a
cookie configured for a Spot Light projects the cookie just once in the direct
path of the “beam” of the light. This is the only case where the position of a
Directional Light is important.  
**Spot Light** | With this type of Light, you want the edges of your Cookie Texture to be solid black to get the proper effect, so Unity sets the **Wrap Mode** to **Clamp** by default.  
**Directional Light** | With this type of Light, this Texture tiles, so Unity sets the **Wrap Mode** to **Repeat** by default.  
**Point Light** | When you choose this type of Light, you can also customize the [Cube shape-specific](class-TextureImporter.html#textureshape) properties.  
