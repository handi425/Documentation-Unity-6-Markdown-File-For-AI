...
Language : English
  ...
  * [English](/Manual/UIE-uxml-element-RepeatButton.html)
  * [中文](/cn/current/Manual/UIE-uxml-element-RepeatButton.html)
  * [日本語](/ja/current/Manual/UIE-uxml-element-RepeatButton.html)
  * [한국어](/kr/current/Manual/UIE-uxml-element-RepeatButton.html)
...
A RepeatButton performs a repetitive action when users press and hold it. You
define the action, a delay, and an interval. When users press the
RepeatButton, the initial delay must be met before the first action starts,
and then it repeats at the specified interval.

**Note** : A RepeatButton doesn’t have the same styling as a regular Button.
You can apply the `.unity-button` USS class to it to make it look like a
regular Button.

## Create a RepeatButton

Although you can create a RepeatButton with UXML, you must create the action
logic in C#. The following C# example creates a RepeatButton that logs a
console message when pressed and held:
